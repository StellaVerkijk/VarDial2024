'''
This code prints an IAA report for documents annotated in Round 2 of the GLOBALISE project, as described in Verkijk, Vossen & Sommerauer, 2024 .
...
df_foelie = post_clean("../annotated_data/IAA/Round2/foelie-triggers-conll.json")
df_kruidnagel = post_clean("../annotated_data/IAA/Round2/kruidnagel-triggers-conll.json")
df_nootmuskaat = post_clean("../annotated_data/IAA/Round2/nootmuskaat-triggers-conll.json")


print('Total amount of spans with an event annotation Foelie: ', len(df_foelie))
print('Total amount of spans with an event annotation  Kruidnagel:', len(df_kruidnagel))
print('Total amount of spans with an event annotation  Nootmuskaat:', len(df_nootmuskaat))
...
def general_statistics(df1, df2):
    ...
    print('Total amount of spans with event annotation Foelie: ', len(df_foelie))
    print('Total amount of spans with event annotation Kruidnagel:', len(df_kruidnagel))
    print('Total amount of spans with event annotation Nootmsuskaat:', len(df_nootmuskaat))
    ...
    print()
    print('Average amount of spans annotated is: ' , ((len(foelie)+len(kruidnagel)+len(nootmuskaat))/3))
...
print("Scores per duo of teams compared to each other for event detection + classification")
table = PrettyTable()
...
table = PrettyTable()
table.field_names = ["", "Only trained teams", "All teams"]
