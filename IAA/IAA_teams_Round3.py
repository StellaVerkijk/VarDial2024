'''
This code prints an IAA report for documents annotated in Round 3 of the GLOBALISE project, as described in Verkijk, Vossen & Sommerauer, 2024
...
df_foelie = post_clean("../annotated_data/IAA/Round3/foelie-triggers-conll.json")
df_kaneel = post_clean("../annotated_data/IAA/Round3/kaneel-triggers-conll.json")
df_kruidnagel = post_clean("../annotated_data/IAA/Round3/kruidnagel-triggers-conll.json")
df_nootmuskaat = post_clean("../annotated_data/IAA/Round3/nootmuskaat-triggers-conll.json")
...
def general_statistics(df1, df2):
    ...
    all_annos = [x for x in foelie + kaneel + kruidnagel + nootmuskaat]
    agreements = [x for x in foelie + kaneel + kruidnagel + nootmuskaat if x in foelie and x in kaneel and x in kruidnagel and x in nootmuskaat]

    print('Number of total spans annotated with an event class: ', len(all_annos))
    print('Number of spans that received an event class label of all annotators: ', len(agreements))
    print('Percentage of mention detection agreement: ', (len(agreements)/len(all_annos))*100)
    print()
    print('Average amount of annotations is: ' , (len(foelie)+len(kaneel)+len(kruidnagel)+len(nootmuskaat))/4)
    print()
...
table4 = PrettyTable()
table4.field_names = ["", "Average class agreement"]
