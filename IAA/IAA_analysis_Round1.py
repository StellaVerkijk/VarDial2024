'''
This code prints an IAA report for documents annotated in Round 1 of the GLOBALISE project as described in Verkijk, Vossen & Sommerauer, 2024.
Round 1 consisted of individual annotation. After annotating individually, annotators checked each others work and any new annotations the annotators wanted to make after this were added for a second layer of analysis.
IAA is calculated with and without resolving non-problematic disagreements through the GLOBALISE event ontology as designed by Verkijk & Vossen, 2023.
...
def post_clean(jsonfile):
    """
    Creating a dataframe with a row per span annotation
    """
...
df_Manjusha = post_clean("../annotated_data/IAA/Round1/json/Manjusha-triggers-conll.json")
df_Brecht = post_clean("../annotated_data/IAA/Round1/json/Brecht-triggers-conll.json")
df_Kay = post_clean("../annotated_data/IAA/Round1/json/Kay-triggers-conll.json")
df_Lodewijk = post_clean("../annotated_data/IAA/Round1/json/Lodewijk-triggers-conll.json")

print('Total amount of spans with event annotation Manjusha: ', len(df_Manjusha))
print('Total amount of spans with event annotation Lodewijk:', len(df_Lodewijk))
print('Total amount of spans with event annotation Brecht: ', len(df_Brecht))
print('Total amount of spans with event annotation Kay:', len(df_Kay))
...
def general_statistics(df_Brecht, df_Kay, df_Manjusha, df_Lodewijk):
    ...
    print('Number of spans that received an event class label of all annotators (exact match): ', len(agreements))
...
def analyse_annotator_pair(df1, df2, annotator1, annotator2, df_check1, df_check2):
    ...
    print('---------------------------------------------------------------------------------------')
    print()
    print("INTERMEDIATE SCORES: before check-task additions")
...
Lcheck1= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Lcheck1.tsv', delimiter='\t', index_col=None)
Lcheck2= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Lcheck2.tsv', delimiter='\t', index_col=None)
Lcheck3= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Lcheck3.tsv', delimiter='\t', index_col=None)

Mcheck2= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Mcheck2.tsv', delimiter='\t', index_col=None)
Mcheck3= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Mcheck3.tsv', delimiter='\t', index_col=None)
Mcheck4= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Mcheck4.tsv', delimiter='\t', index_col=None)

Kcheck1= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Kcheck1.tsv', delimiter='\t', index_col=None)
Kcheck3= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Kcheck3.tsv', delimiter='\t', index_col=None)
Kcheck4= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Kcheck4.tsv', delimiter='\t', index_col=None)

Bcheck1= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Bcheck1.tsv', delimiter='\t', index_col=None)
Bcheck2= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Bcheck2.tsv', delimiter='\t', index_col=None)
Bcheck4= pd.read_csv('../annotated_data/IAA/Round1/checked_data/Bcheck4.tsv', delimiter='\t', index_col=None)
...
########### Get coverage disagreements ### I THINK THIS CAN BE DELETED
...
table = PrettyTable()
table.field_names = ["", "Before check-task", "After check-task"]
